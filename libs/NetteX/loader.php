...
dumpX($var){foreach(func_get_args()as$arg)NetteX\Debug::dump($arg);return$var;}}namespace NetteX{use
NetteX;interface
IComponent{const
...
callbackX($m[1][0]===':'?array($This,substr($m[1],1)):$m[1])->invoke($content,$modifiers);}else{return$This->formatMacroArgs($content,'#');}});}function
...
new\LogicException("Cannot instantiate static class ".get_class($this));}static
function
loader($helper){$callback=callbackX('NetteX\Templates\TemplateHelpers',$helper);if($callback->isCallable()){return$callback;}$callback=callbackX('NetteX\String',$helper);if($callback->isCallable()){return$callback;}}static
function
...
new
AmbiguousServiceException("Cannot instantiate service '$name', class '$factory' not found.");}$service=new$factory;if($options&&method_exists($service,'setOptions')){$service->setOptions($options);}}else{$factory=callbackX($factory);if(!$factory->isCallable()){throw
